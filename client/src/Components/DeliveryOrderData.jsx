...
const DeliveryOrderData = ({ index, data, admin }) => {
  ...
  const handleVerifyOTP = async (orderId, sts) => {
    try {
      ...
      const response = await verifyOtp(enteredOtp, randomOtp);
      if (response) {
        setShowModal(false);
    ...
    <>
      ...
      <div className="flex items-center justify-center min-h-screen">
        ...
        <div className="relative z-50 bg-white rounded-lg max-w-sm p-6">
          <span
            className="absolute top-0 right-0 cursor-pointer mr-4 mt-4 text-gray-600 hover:text-gray-800"
            ...
            <div className="flex justify-center mb-4">
              {[...Array(6)].map((_, index) => (
                <input
                  ...
                  className="w-10 h-10 text-center mx-1 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500"
                />
              ))}
            </div>
            <button
              onClick={() => handleVerifyOTP(data.orderId, "Delivered")}
              className={`w-full bg-gradient-to-r from-yellow-500 via-orange-500 to-orange-600 text-white py-2 rounded-md focus:outline-none hover:bg-orange-700`}
            >
              Verify OTP
            </button>
          </div>
